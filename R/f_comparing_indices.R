...
compare_indices_df <-
  function(
    ...
    ggplot2::ggplot(data, ggplot2::aes(x = time, y = value, col = formula)) +
      ggplot2::geom_point() +
      ggplot2::geom_line() +
      ggplot2::labs(x = "date", y = "price index value") +
...
#' \donttest{
#' index1 <- final_index(
#'   data = milk, start = "2018-12",
#'   end = "2019-12", formula = "walsh", interval = TRUE
#' )
#' }
#' \donttest{
#' index2 <- price_indices(milk,
#'   start = "2018-12", end = "2019-12",
...
compare_indices_list <- function(data = list(), names = c(), date_breaks = "1 month") {
  ...
  for (m in 1:length(data)) {
    if (nrow(data.frame(data[[m]])) == 0) stop("At least one data frame is empty")
  }
  for (m in 1:length(data)) {
    if (!(nrow(data.frame(data[[m]])) == nrow(data.frame(data[[1]])))) stop("Data frames must have identical number of rows")
  }
  ...
  ggplot2::ggplot(graph, ggplot2::aes(x = time, y = value, col = formula)) +
    ggplot2::geom_point() +
    ggplot2::geom_line() +
    ggplot2::labs(x = "date", y = "price index value") +
    ggplot2::scale_x_date(date_labels = "%Y %m", date_breaks = date_breaks) +
...
compare_to_target <- function(data = data.frame(), target, measure = "MAD", pp = TRUE, first = TRUE, prec = 3) {
  ...
  if (is.data.frame(target)) {
    ...
    if (is.numeric(target[, 1])) {
      target <- target[, 1]
    } else if (is.numeric(target[, 2])) {
      target <- target[, 2]
    } else {
      stop("Data frame 'target' should contain one numeric column with target index values!")
    }
  }
  ...
  for (i in 1:ncol(data)) {
    ...
    # distance
    if (measure == "MAD") {
      dist <- MAD(x, target, type = "all")
    } else {
      dist <- MSD(x, target, type = "all")
    }
    ...
    # distance_upper
    if (measure == "MAD") {
      dist2 <- MAD(x, target, type = "upper")
    } else {
      dist2 <- MSD(x, target, type = "upper")
    }
    ...
    # distance_lower
    if (measure == "MAD") {
      dist3 <- MAD(x, target, type = "lower")
    } else {
      dist3 <- MSD(x, target, type = "lower")
    }
...
compare_indices_jk <- function(data,
  ...
  figure <- ggplot2::ggplot(
    data = df_index,
    mapping = ggplot2::aes(x = variable, y = value)
  ) +
    ggplot2::geom_boxplot() +
    ggplot2::xlab("price index") +
    ggplot2::ylab("price index value")
