...
data_matching <-
  function(
    ...
    if (codeIN == TRUE & codeOUT == TRUE & description == TRUE) {
      if (length(variables) > 0) {
        pairs <- reclin2::pair_blocking(data, on = variables, deduplication = TRUE)
      } else {
        pairs <- reclin2::pair(data, deduplication = TRUE)
      }
...
matched_fig <-
  function(
    ...
    if (figure == FALSE) {
      ...
      ggplot2::ggplot(tab, ggplot2::aes(x = date, y = fraction)) +
        ggplot2::geom_point() +
        ggplot2::geom_line() +
        ggplot2::labs(x = "date", y = "fraction") +
        ggplot2::scale_x_date(
          date_labels =
            "%Y %m", date_breaks = date_breaks
        ) +
...
prices <- function(data, period, set = c(), ID = FALSE) {
  ...
  if (length(set) > 0) {
    data <- dplyr::filter(data, prodID %in% set)
    if (nrow(data) == 0) {
      stop("There are no data in selected period")
    }
  }
...
pqcor <- function(
  ...
  if (figure == TRUE) {
    ...
    ggplot2::ggplot(df, ggplot2::aes(x = prices, y = quantities)) +
      ggplot2::geom_point() +
...
pqcor_fig <- function(data,
  ...
  if (figure == FALSE) {
    ...
    tab$date <- as.Date(paste(tab$date, "01", sep = "-"))
    ggplot2::ggplot(tab, ggplot2::aes(x = date, y = correlation)) +
      ggplot2::geom_point() +
      ggplot2::geom_line() +
      ggplot2::labs(x = "date", y = "correlation") +
...
#' \donttest{
#' sales_groups(
#'   datasets = list(milk1, milk2, milk3), start = "2019-04", end = "2019-07",
...
#' @examples
#' generate(
#'   pmi = c(1.02, 1.03, 1.04), psigma = c(0.05, 0.09, 0.02), qmi = c(3, 4, 4),
#'   qsigma = c(0.1, 0.1, 0.15), start = "2020-01", days = TRUE
#' )
#' \donttest{
#' generate(
#'   pmi = c(1.02, 1.03, 1.04), psigma = c(0.05, 0.09, 0.02), qmi = c(6, 6, 7),
...
#' \donttest{
#' df <- generate_CES(
#'   pmi = c(1.02, 1.03), psigma = c(0.04, 0.03),
    ...
    if (benchmark == "end")
      {{ t2 <- substr(end, 0, 7)
        ...
        if (figure == FALSE) {
          ...
          tab$date <- as.Date(paste(tab$date, "01", sep = "-"))
          ggplot2::ggplot(tab, ggplot2::aes(x = date, y = dissimilarity)) +
            ggplot2::geom_point() +
            ggplot2::geom_line() +
            ggplot2::labs(x = "date", y = "dissimilarity") +
            ggplot2::scale_x_date(date_labels = "%Y %m", date_breaks = date_breaks) +
            ggplot2::theme(axis.text.x = ggplot2::element_text(angle = 45, hjust = 1))
        } }} else {
      values <- c(values, 0)
...
#' \donttest{
#' sales_groups2(milk,
#'   by = "retID", start = "2019-04", end = "2019-04",
...
data_aggregating <- function(data, join_outlets = TRUE) {
  ...
  if (join_outlets == TRUE) {
    data_aggr <- dplyr::summarise(dplyr::group_by(data, time, prodID), prices2 = sum(prices * quantities) / sum(quantities), quantities2 = sum(quantities), .groups = "drop")
  } else {
...
elasticity <- function(data, start, end, method = "lm", left = -10, right = 10, precision = 0.000001) {
  ...
  superlative <- function(sigma) {
    ...
    if (method == "f") {
      return((sum(s_start * (p_end / p_start)^(1 - sigma)))^(1 / (1 - sigma)) - (sum(q_start * p_end) * sum(q_end * p_end) / (sum(q_start * p_start) * sum(q_end * p_start)))^0.5)
    }
    if (method == "t") {
      return((sum(s_start * (p_end / p_start)^(1 - sigma)))^(1 / (1 - sigma)) - prod((p_end / p_start)^(0.5 * (s_start + s_end))))
    }
    if (method == "w") {
      return((sum(s_start * (p_end / p_start)^(1 - sigma)))^(1 / (1 - sigma)) - sum(p_end * (q_start * q_end)^0.5) / sum(p_start * (q_start * q_end)^0.5))
    }
    if (method == "sv") {
      return((sum(s_start * (p_end / p_start)^(1 - sigma)))^(1 / (1 - sigma)) - sv.)
    }
  }
  ...
  while (abs(superlative(x0)) > precision) {
    if (superlative(ll) * superlative(x0) > 0) {
      ll <- x0
    } else {
      pp <- x0
    }
...
#' \donttest{
#' elasticity_fig(milk,
#'   start = "2018-12", end = "2019-04", figure = TRUE,
...
elasticity_fig <- function(data, start, end, method = c("lm"), fixedbase = TRUE, figure = TRUE, date_breaks = "1 month", names = c(), left = -10, right = 10, precision = 0.000001) {
  ...
  if (figure == FALSE) {
    ...
    fig <- ggplot2::ggplot(df, ggplot2::aes(x = date, y = value, col = formula)) +
      ggplot2::geom_point() +
      ggplot2::geom_line() +
      ggplot2::labs(x = "date", y = "elasticity of substitution") +
      ggplot2::scale_x_date(date_labels = "%Y %m", date_breaks = date_breaks) +
      ggplot2::theme(axis.text.x = ggplot2::element_text(angle = 45, hjust = 1))
    return(fig)
  }
...
#' # Price imputing:
#' data_imputing(my_data,
#'   start = "2018-12", end = "2019-01",
...
products <- function(data, start, end) {
  ...
  summary_prodID <- list(
    available_prodID = available_prodID,
  ...
  df_prodID <- data.frame(
    ...
    volume = c(
      length(available_prodID), length(matched_prodID),
      length(new_prodID), length(disappearing_prodID)
    ),
    shares = c(
      100, round(100 * length(matched_prodID) / length(available_prodID), 2),
...
products_fig <- function(data,
  ...
  f_base_help <- function(t) {
    df_prod <- products(data = data, start = dates[1], end = dates[t])$statistics
    df_prod$time <- dates[t]
    return(df_prod)
  }
  # helping function for the chain case
  f_chain_help <- function(t) {
    df_prod <- products(data = data, start = dates[t - 1], end = dates[t])$statistics
    df_prod$time <- dates[t]
    return(df_prod)
  }
  # creating a data frame with results
  if (fixed_base == TRUE) {
    list_df <- lapply(seq(2, length(dates)), f_base_help)
  } else {
    list_df <- lapply(seq(2, length(dates)), f_chain_help)
  }
  ...
  if (contributions == TRUE) { # graphical presentation of shares
    figure <- ggplot2::ggplot(df, ggplot2::aes(x = time, y = shares, col = products)) +
      ggplot2::geom_point() +
      ggplot2::geom_line() +
      ggplot2::labs(x = "date", y = "contribution value [%]") +
      ggplot2::scale_x_date(date_labels = "%Y %m", date_breaks = date_breaks) +
      ggplot2::theme(axis.text.x = ggplot2::element_text(angle = 45, hjust = 1))
  } else { # graphical presentation of volumes
    figure <- ggplot2::ggplot(df, ggplot2::aes(x = time, y = volume, col = products)) +
      ggplot2::geom_point() +
      ggplot2::geom_line() +
...
data_reducing <- function(data, start, end, type = "prodID", minN = 2, outlets = FALSE, by = c(), interval = FALSE) {
  ...
  # list of subgroups
  if (outlets == FALSE) {
    subgroup_list <- base::split(x = data, f = data[, by], drop = TRUE)
  } else {
    subgroup_list <- base::split(x = data, f = list(data[, by], data$retID), drop = TRUE)
  }
  # checking if there are at least minN products
  if (interval == TRUE) { # each pair of subsequent months is considered
  ...
  } else { # only the base and current periods are considered
    flags <- c()
    for (i in 1:length(subgroup_list)) {
      if (length(matched(subgroup_list[[i]],
        period1 = substr(as.character(start), 0, 7),
        period2 = substr(as.character(end), 0, 7), type = type
      )) < minN) {
        flags <- c(flags, i)
      }
    }
...
#' df <- data_matching(
#'   data = data_DOWN_UP_SIZED, start = "2024-01", end = "2024-02",
...
#' result <- shrinkflation(
#'   data = df, start = "2024-01", "2024-02",
...
shrinkflation <- function(data, start, end, type = "shrinkflation",
  ...
  potential_types <- c(
    "shrinkflation", "shrinkdeflation", "sharkflation",
  ...
  changes <- dplyr::summarise(dplyr::group_by(changes, prodID, time),
    ...
    price_orig = replicate_str(
      length(unique(grammage)),
      paste(unique(mean_price_orig), collapse = " , "),
      unique(mean_price_orig)
    ),
    price_norm = replicate_str(
      length(unique(grammage)),
  ...
  for (i in 1:length(list_potential)) {
    ...
    for (k in 1:nrow(frame)) {
      if (k == 1) {
        ...
        dates <- c(dates, replicate(
          length(ratios(x_size, y_size)),
          paste(frame[k - 1, "time"], frame[k, "time"], sep = " , ")
        ))
        descriptions <- c(descriptions, replicate(
          length(ratios(x_size, y_size)),
