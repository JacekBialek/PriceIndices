...
price_indices <-
  function(
    ...
    considered_indices <- c(
      inc_splice, inc_window, inc_base,
    ...
    if (interval == FALSE) {
      ...
      if (length(names) > 0) {
        df <- data.frame(price_index = names, value = unlist(results_list))
      } else {
