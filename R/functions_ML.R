...
model_classification<-function(data_train=data.frame(), data_test=data.frame(), indicators=c(), key_words=c(), sensitivity=TRUE, p=0.9, w=0.2, rounds=200, grid=list())
{ #default value of 'grid'
  grid_default<-list(eta=c(0.05,0.1,0.2),max_depth=c(6),min_child_weight=c(1),max_delta_step=c(0),subsample=c(1),gamma=c (0),lambda=c(1),alpha=c(0))
  ng_ok<-names(grid_default)
  ng<-names(grid)
  if (length(ng)>0) {for (i in 1:length(ng)) {if (ng[i] %in% ng_ok) grid_default[[ng[i]]]<-grid[[ng[i]]]
                                              else stop(paste("The 'grid' list does not include the parameter: ",ng[i]))
                                             }
                    }
  grid<-grid_default
  #checking parameters
  if (nrow(data_train)==0) stop("There is no data_train among parameters or the data_train set is empty!")
  if ((length(indicators)+length(key_words))==0) stop ("The model cannot be built due to the lack of indicators")
  cn<-colnames(data_train)
  if (!("coicop" %in% cn)) stop("A column 'coicop' is missing")
  if (("description" %in% indicators) | (length(key_words)>=1)) {
       if (!(("description" %in% cn))) stop("A column 'description' is missing!")
  }
description<-coicop<-iterations<-value<-error<-Feature<-Importance<-NULL
if (nrow(data_test)==0) {
#Creating data_train and data_test
trainIndex <- caret::createDataPartition(data_train$coicop,p=p,list=FALSE) 
...
data_train <- data_train[trainIndex,] 
  ...
  #lower/uppar cases
  if (sensitivity==TRUE) {if (("description" %in% indicators) | (length(key_words)>=1))
     {
     data_train$description<-tolower(data_train$description)
     data_test$description<-tolower(data_test$description)
     }
   if (length(key_words)>=1) key_words<-tolower(key_words)
                            
                          } 
  
  #data_train reduction
  data_train<-dplyr::select(data_train,indicators,description,coicop)
  ...
  if (length(key_words)>=1) for (i in 1:length(key_words)) {var<-stringr::str_detect(data_train$description, pattern=key_words[i])
                                  data_train[,key_words[i]]<-as.numeric(var)
                                 }
  
  if (!("description" %in% indicators)) data_train$description<-NULL 
  cols_train<-colnames(data_train)
  for (i in 1:length(cols_train)) if (is.numeric(data_train[,cols_train[i]])==0) data_train[,cols_train[i]]<-as.numeric(as.factor(data_train[,cols_train[i]]))
  X <- data.matrix(data_train)
  
  #data_test preparation
  Y_test<-as.numeric(as.factor(data_test$coicop))-1
  data_test$coicop<-NULL
  if (length(key_words)>=1) for (i in 1:length(key_words)) {var<-stringr::str_detect(data_test$description, pattern=key_words[i])
                                  data_test[,key_words[i]]<-as.numeric(var)
                                 }
  
  if (!("description" %in% indicators)) data_test$description<-NULL 
  cols_test<-colnames(data_test)
  for (i in 1:length(cols_test)) if (is.numeric(data_test[,cols_test[i]])==0) data_test[,cols_test[i]]<-as.numeric(as.factor(data_test[,cols_test[i]]))
  ...
  models<-list() #list of created models (best for the given set of parameters)
 ...
 xgb.fit <- xgboost::xgb.train(params = param, 
  ...
  #best iteration
  train_error<-xgb.fit$evaluation_log[xgb.fit$best_iteration,]$train_merror
  test_error<-xgb.fit$evaluation_log[xgb.fit$best_iteration,]$test_merror
  ...
  parameter_list[[length(parameter_list)+1]]<-list(p=p,w=w,rounds=rounds,eta=grid$eta[p1],max_depth=p2,min_child_weight=grid$min_child_weight[p3],max_delta_step=grid$max_delta_step[p4],subsample=grid$subsample[p5],gamma=grid$gamma[p6],lambda=grid$lambda[p7],alpha=grid$alpha[p8])
               }
  ...
  figure_training<-ggplot2::ggplot(df_errors, ggplot2::aes(x=iterations, y=value, col=error))+ggplot2::guides(color = ggplot2::guide_legend(override.aes = list(size = 3)))+ggplot2::geom_point(size=0.9)+ggplot2::geom_line(show.legend =FALSE)+ggplot2::labs(y="error value")

  #importance of factors
  impMatrix <- data.frame(xgboost::xgb.importance(model = xgb.fit))
  df_imp<- data.frame(Feature=impMatrix$Feature,Importance=impMatrix$Gain)
  figure_importance<-ggplot2::ggplot(data=df_imp, ggplot2::aes(x=Feature, y=Importance))+ggplot2::coord_flip()+
  ggplot2::geom_bar(stat="identity", fill="grey",color="black")+ggplot2::labs(x="Relative importance")
 
return (list(model=xgb.fit,best_parameters=best_parameters, indicators=indicators,key_words=key_words,coicops=coicops,sensitivity=sensitivity,figure_training=figure_training,figure_importance=figure_importance))
}
...
{
description<-NULL
if (nrow(data)==0) stop("The dataset is empty")
if (!(length(model)==8)) stop ("The model appears to be incomplete")
...
if (length(key_words)>=1) for (i in 1:length(key_words)) 
     {var<-stringr::str_detect(data_new$description,  pattern=key_words[i])
      data_new[,key_words[i]]<-as.numeric(var)
     }
if (!("description" %in% indicators)) data_new$description<-NULL 
cols_new<-colnames(data_new)
for (i in 1:length(cols_new)) if (is.numeric(data_new[,cols_new[i]])==0) data_new[,cols_new[i]]<-as.numeric(as.factor(data_new[,cols_new[i]]))
...
predictions_new<-c(predictions_new, as.vector(coicops$coicop_oryg)[no])
                                    }
...
save_model<-function (model=list(), dir="ML_model")
{
 if (!(length(model)==8)) stop ("The model appears to be incomplete") 
  ...
  saveRDS(figure_importance,paste(path,"figure_importance.RDS",sep=""))
}
...
load_model<-function (dir="ML_model")
{
  ...
  figure_importance<-readRDS(paste(path,"figure_importance.RDS",sep=""))
  return (list(model=model_used,best_parameters=best_parameters,
  indicators=indicators,key_words=key_words,coicops=coicops,sensitivity=sensitivity,
  figure_training=figure_training,figure_importance=figure_importance))
  }
