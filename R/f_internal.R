    ...
    val <- function(id) {
      xx <-
        function(tt) {
          ...
          if (nrow(gr_subset1) > 0) {
            return(sum(gr_subset1$expend) / index1[which(dates == tt)])
          } else {
            return(0)
          }
        }
      yy <-
        function(tt) {
          ...
          if (nrow(gr_subset2) > 0) {
            return(sum(gr_subset2$quant))
          } else {
            return(0)
          }
        }
...
unit <-
  function(
    ...
    pattern. <- paste("\\d*[[:space:]]*",
      multiplication,
      "{0,1}[[:space:]]*[0-9]+[[:punct:]]{0,1}[0-9]*[[:space:]]*(", units, ")[[:space:]]+",
      sep = ""
    )
...
final_index2 <-
  function(
    ...
    if (aggr == "laspeyres") {
      return(sum(weights_start * indices))
    }
    if (aggr == "paasche") {
      return(1 / sum(weights_end * (1 / indices)))
    }
    if (aggr == "fisher") {
      return(((1 / sum(weights_end * (1 / indices))) * sum(weights_start * indices))^0.5)
    }
    if (aggr == "tornqvist") {
      return(prod(indices^(0.5 * (weights_start + weights_end))))
    }
    if (aggr == "arithmetic") {
      return(mean(indices))
    }
    if (aggr == "geometric") {
      return((prod(indices))^(1 / length(indices)))
    }
  }
...
bennet_internal <-
  function(
    ...
    dates <- seq.Date(from = start, to = end, by = "month")
    pq <- function(tm) {
      frame <- prices(data, period = tm, ID = TRUE)
...
mbennet_internal <-
  function(
    ...
    setID <- NULL
    if (matched == FALSE) {
      setID <- available(data, period1 = wstart, period2 = wend, interval = TRUE)
    } else {
      setID <- matched(data, period1 = wstart, period2 = wend, interval = TRUE)
    }
